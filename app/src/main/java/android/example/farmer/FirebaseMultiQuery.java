...
public class FirebaseMultiQuery {
    ...
    private static volatile FirebaseMultiQuery sSoleInstance;
    ...
    public void setRefs(final DatabaseReference... refs){
        for (final DatabaseReference ref : refs){
            add(ref);
        }
    }

    private FirebaseMultiQuery() {
    ...
    public static FirebaseMultiQuery getInstance(){
        //Double check locking pattern
        if (sSoleInstance == null) { //Check for the first time

            synchronized (DatabaseSingleton.class) {   //Check for the second time.
                //if there is no instance available... create new one
                if (sSoleInstance == null) sSoleInstance = new FirebaseMultiQuery();
            }
        }
